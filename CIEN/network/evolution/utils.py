...
def normalize_poly(polys):
    min_x, min_y = torch.min(polys[..., 0], dim=-1, keepdim=True)[0], \
        torch.min(polys[..., 1], dim=-1, keepdim=True)[0]
    max_x, max_y = torch.max(polys[..., 0], dim=-1, keepdim=True)[0], \
        torch.max(polys[..., 1], dim=-1, keepdim=True)[0]
    len_x, len_y = max_x - min_x, max_y - min_y
    norm_polys = polys.clone()
    norm_polys[..., 0] = (norm_polys[..., 0] - min_x) / len_x
    norm_polys[..., 1] = (norm_polys[..., 1] - min_y) / len_y
    return norm_polys.detach()