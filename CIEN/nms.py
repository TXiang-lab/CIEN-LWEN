...
def post_process(output, cfg):
    detection = output['detection']
    score = detection[:, 2].detach()
    label = detection[:, 3].detach()
    # last_py = output['poly_coarse'].detach()
    # last_py = output['poly_final'].detach()
    stage = {'init': 'poly_init', 'coarse': 'poly_coarse', 'final': 'poly_final'}
    s = stage[cfg.test.test_stage]
    last_py = output[s].detach()
    if len(last_py) == 0:
        return 0
